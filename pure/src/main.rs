use std::{collections::HashMap, env, fs::File, io::Read, sync::mpsc, thread};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    thread::spawn(move || {
        while let Ok(new_results) = rx.recv() {
    ...
    loop {
        ...
        spawns.push(thread::spawn(move || spawned_working(chunk, tx_clone)));
...
fn spawned_working(chunk: Vec<u8>, tx: mpsc::Sender<HashMap<String, (f64, f64, f64, u32)>>) {
    let mut results: HashMap<String, (f64, f64, f64, u32)> = HashMap::default();
    let mut name = "";
    let mut idx = 0;
